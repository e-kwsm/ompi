...
add_header_first()
{
    ...
    if [ $prev_line = -1 ] || [ $prev_line = 0 ] ; then
      prev_line=1
    fi
...
for i in $(find . -type f '(' -name '*.[cChysSfF]' -o -iname '*.cc' -o -name '*.cpp' -o -name '*.[fF]77' -o -name '*.[fF]90' ')' | sort | xargs grep -n $SEARCH_HEADER | cut -f1 -d':' | sort | uniq) ; do
    ...
    if [ $need_opal_util_output_h -eq 1 ] && [ $found_opal_util_output_h -eq 0 ] ; then
    ...
    if [ $need_orte_mca_rml_rml_types_h -eq 1 ] && [ $found_orte_mca_rml_rml_types_h -eq 0 ] ; then
